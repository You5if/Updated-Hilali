...
import { JournalDynamicEntryComponent } from './stockin-entry/stockin-entry.component';
...
export class JournalDynamicComponent implements OnInit {
  ...
  ngOnInit() {
    this.titleService.setTitle("Journal Entry - Elhelali");
      this.refreshMe();
  }
  ...
  refreshMe() {
    ...
    if (localStorage.getItem(this._globals.baseAppName + '_language') == "16001") {
      this.direction = "ltr"
      this.header = "Journal Entry"
      this.stockInDate = "Stock in date"
      this.stockInCode = "JournalId"
  ...
  openEntry   (result: StockInModel) {
    if (result === undefined) {
      const dialogRef = this.dialog.open(JournalDynamicEntryComponent, {
        disableClose: true,
        data: {}
      });
