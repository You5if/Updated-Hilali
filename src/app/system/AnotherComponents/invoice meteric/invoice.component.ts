...
export class InvoiceMeterComponent implements OnInit {
  ...
  refreshMe() {
    ...
    if (this._cf.filterVar != '') {
      this.pageData.filter = "isCubicMeasure = '1'" + ' ' + 'and' + ' ' + this._cf.filterVar
    }else {

      this.pageData.filter = "isCubicMeasure = '1'"
    }
  ...
  openEntry   (result: InvoiceModel) {
    if (result === undefined) {
      const dialogRef = this.dialog.open(InvoiceMeterEntryComponent, {
        disableClose: true,
        data: {}
      });
      dialogRef.afterClosed().subscribe(() => {
