...
export class InvoiceMeterEntryComponent implements OnInit {
  url!: string;
  total!: number;
  totalTax!: number;
  totalE!: string;
  ...
  constructor( private dapiService: InvoiceEntryService,
     private _ui: UIService,
      ...
      private dialogRef: MatDialogRef<InvoiceMeterEntryComponent>,
  ...
  ngOnInit(): void {
    ...
    if(localStorage.getItem(this._globals.baseAppName + '_language') == "16001") {
      ...
      this.width = "Width"
      this.height = "Height"
      this.cubicUnit = "Cubic unit"
      this.cubicPrice = "Cubic meter price"
      this.weight = "Weight"
      ...
      this.direction = "rtl"
      this.submit = "ارسال"
      this.thickness = "السمك"
      this.width = "العرض"
      this.height = "الارتفاع"
      this.cubicUnit = "وحدة المتر المربع"
      this.cubicPrice = "سعر المتر المربع"
      this.weight = "الوزن"
      this.unitPrice = "سعر الوحدة"
      this.totalPrice = "السعر الكلي"
      this.cancel = "الغاء"
  ...
  addChild1ExpenseItem(id:number) {  
    ...
    this.dapiService.child1Controllers(this.model2).subscribe((res) => {
      ...
      this.childElemInit[4].access = "NoAccess"
      this.childElemInit[9].access = "NoAccess"
      this.childElemInit[10].value = "1"
  ...
  onAmountChange(e: Event, id:any): void {
    ...
    console.log(this.lastDark.child1[0].records[3]);
      ...
      this.weightN = this.thickN * this.widthN * this.heightN * Number( this.lastDark.child1[id].records[3].value)
      this.lastDark.child1[id].records[5].value = this.thickN * this.widthN * this.heightN * this.cubicPN
      this.lastDark.child1[id].records[6].value = this.thickN * this.widthN * this.heightN *Number( this.lastDark.child1[id].records[3].value) * this.cubicPN
    // this.subTotal = 0
    // for (let i = 0; i < this.lastDark.child1.length; i++) {
    //   if (this.lastDark.child1[i] && this.lastDark.child1[i].records[3].tableColumnId == 291) {
    //     this.subTotal += (this.lastDark.child1[i].records[3].value * this.lastDark.child1[i].records[5].value )
  ...
  onChangeValue(id: number, id2: number) {
    ...
    this.dapiService.getProductMeter(id).subscribe((resu: any) => {
      this._ui.loadingStateChanged.next(false);
      console.log("Meter", resu);
      this.meterArray = resu
      this.thickN = resu.thickN
      this.widthN = resu.widthN
      this.heightN = resu.heightN
      this.cubicUN = resu.thickN * resu.widthN * resu.heightN 
      this.cubicPN = resu.cubicMeterCost
      this.weightN = resu.thickN * resu.widthN * resu.heightN * Number( this.lastDark.child1[id2].records[3].value)
      this.lastDark.child1[id2].records[5].value = this.thickN * this.widthN * this.heightN * this.cubicPN
      this.lastDark.child1[id2].records[6].value = this.thickN * this.widthN * this.heightN *Number( this.lastDark.child1[id2].records[3].value) * this.cubicPN
      // for (let i = 0; i < this.lastDark.child1.length; i++) {
      //   if (this.lastDark.child1[i] == this.lastDark.child1[id2]) {
      //     this.lastDark.child1[i].records[6].value = this.lastDark.child1[i].records[3].value * this.lastDark.child1[i].records[5].value
